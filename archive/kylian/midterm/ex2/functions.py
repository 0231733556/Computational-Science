...
def ex1_objective(u: np.ndarray) -> float:
    """
    Objective function:
        f(u) = -u_n + (u_1)^4 + sum_{j=2}^{n} (u_j - u_{j-1})

    Args:
        u (np.ndarray): Vector of variables [u_1, u_2, ..., u_n].
    ...
    if n < 2:
        raise ValueError("u must have at least two elements")

    # Compute each term
    term1 = -u[-1]
    term2 = u[0] ** 4
    term3 = np.sum(u[1:] - u[:-1])

    # Combine terms
    f = term1 + term2 + term3
    return f

def ex1_gradient(u: np.ndarray) -> np.ndarray:
    """
    Gradient of f(u) = -u_n + (u_1)^4 + sum_{j=2}^{n} (u_j - u_{j-1})^4

    Args:
        u (np.ndarray): 1D array of variables [u1, u2, ..., un]

    Returns:
        np.ndarray: Gradient vector ∇f(u)
    """
    u = np.asarray(u, dtype=float)
    n = u.size
    if n < 1:
    ...
    # Derivative of (u1)^4 term
    grad[0] += 4 * u[0] ** 3

    # Derivatives of sum (u_j - u_{j-1})^4
    for j in range(1, n):
        diff = u[j] - u[j-1]
        grad[j] += 4 * diff ** 3       # derivative wrt u_j
        grad[j-1] -= 4 * diff ** 3     # derivative wrt u_{j-1}

    # Derivative of -u_n term
    grad[-1] -= 1.0

    return grad

def ex1_hessian(u: np.ndarray) -> np.ndarray:
    """
    Hessian of f(u) = -u_n + (u_1)^4 + sum_{j=2}^{n} (u_j - u_{j-1})^4

    Args:
        u (np.ndarray): 1D array of variables [u1, u2, ..., un]

    Returns:
        np.ndarray: Hessian matrix ∇²f(u)
    """
    u = np.asarray(u, dtype=float)
    n = u.size
    if n < 1:
    ...
    # Second derivative of (u1)^4 term
    H[0, 0] += 12 * u[0] ** 2

    # Second derivatives from sum (u_j - u_{j-1})^4
    for j in range(1, n):
        diff = u[j] - u[j-1]
        val = 12 * diff ** 2
        H[j, j]     += val
    ...
    # Derivative of -u_n term has no second derivative
